        ...
        if (authorizeAttributes.Any())
        {
            ...
            if (authorizeAttributesWithRoles.Any())
            {
                ...
                foreach (string[]? roles in authorizeAttributesWithRoles.Select(a => a.Roles.Split(',')))
                {
                    ...
                    {
                        bool isInRole = await _identityService.IsInRoleAsync(_user.Id ?? Guid.Empty, role.Trim());
                        if (isInRole)
                        {
                            authorized = true;
                            break;
            ...
            if (authorizeAttributesWithPolicies.Any())
            {
                ...
                {
                    bool authorized = await _identityService.AuthorizeAsync(_user.Id ?? Guid.Empty, policy);
